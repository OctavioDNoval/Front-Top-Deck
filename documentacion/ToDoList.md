# To Do List (Funcionalidad Completa)

- ## Front
    - ### Estilos
        - [ ] hacer mobile para el carrusel (_HomePage_)
        - [ ] mejorar Media Queries del footer (_FooterComponent_)
        - [ ] Footer link a _blank (_FooterComponent_)
        - [ ] Hacer todas las Media Queries de la pagina de productos (_ProductPage_)
        - [ ] Sacar botones de (+) en las tarjetas de productos (_ProductCard_)
        - [ ] Hacer Media Queries de la pagina de producto seleccionada (_SelectedProductPage_)
        - [ ] Hacer que los precios aparezcan en blanco (_SelectedProductPage_)
        - [ ] Media Queries del carrito? (_CarritoComponent_)
        - [ ] Rehacer el componente que muestra el Perfil (_ProfileComponent_)
        - [ ] Mostrar los datos de la cuenta como telefono (_ProfileComponent_)
        - [ ] Mostrar las direcciones guardadas del usuario (_ProfileComponent_)

    - ### Funcionalidades
        - [ ] Paginacion de productos (_ProductPage_) 
        - [ ] Autoseleccion de la direccion principal en base al campo de la base de datos (_PedidoPage / DireccionCard_)
        - [ ] Modal para terminos de uso y condiciones (_ModalTerminosComponent_)
            - [ ] actualizacion manual?
            - [ ] actaulizacion en BDD?
        - [ ] Arreglar mensaje de WP, no se ven los emojis, arreglar eso (_useWhatsApp_)


- ## Back
    - ### Pedidos
        - [ ] Al confirmar el pedido guardarlo en la base de datos
    - ### Direcciones
        - [ ] Borrar Direccion


- ## Base De Datos
    - ## Tablas
        - [ ] agregar a la tabla de pedidos los campos de IP del usuario que pidio, terminos aceptados (booleano), version de los terminos aceptados (_Tabla Pedidos_)